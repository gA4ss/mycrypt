...
namespace mycrypt
{
  // static const unsigned int sm4_encrypt = 1;
  // static const unsigned int sm4_decrypt = 0;
  ...
  */
  ...
  static inline uint32_t __rotl(uint32_t a, uint8_t n)
  {
      return (a << n) | (a >> (32 - n));
  }

  static inline uint32_t __load_u32_be(const uint8_t *b, uint32_t n)
  {
  ...
  static inline void __store_u32_be(uint32_t v, uint8_t *b)
  {
    b[0] = (uint8_t)(v >> 24);
    b[1] = (uint8_t)(v >> 16);
    b[2] = (uint8_t)(v >> 8);
  ...
  static inline uint32_t sm4_t_slow(uint32_t X)
  {
    ...
    // L 线性变换
    return t ^ __rotl(t, 2) ^ __rotl(t, 10) ^ __rotl(t, 18) ^ __rotl(t, 24);
  }
  ...
  int sm4_set_key(const uint8_t *key, sm4_key_t &ks)
  {
    ...
    K[0] = __load_u32_be(key, 0) ^ FK[0];
    K[1] = __load_u32_be(key, 1) ^ FK[1];
    K[2] = __load_u32_be(key, 2) ^ FK[2];
    K[3] = __load_u32_be(key, 3) ^ FK[3];

    for (unsigned int i = 0; i != sm4_key_schedule; ++i) {
...
#define sm4_rnds(k0, k1, k2, k3, F)          \
  ...
  {
    uint32_t B0 = __load_u32_be(in, 0);
    uint32_t B1 = __load_u32_be(in, 1);
    uint32_t B2 = __load_u32_be(in, 2);
    uint32_t B3 = __load_u32_be(in, 3);
    ...
    __store_u32_be(B3, out);
    __store_u32_be(B2, out + 4);
    __store_u32_be(B1, out + 8);
    __store_u32_be(B0, out + 12);
  }
