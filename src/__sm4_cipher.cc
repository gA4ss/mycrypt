...
typedef struct __sm4_cipher_t
{
  ...
  __sm4_cipher_t()
  {
    ...
    memset(final, 0, max_block_length);

    block = nullptr;
    stream.fptr = nullptr;
  }

  int init(int m, const unsigned char *key, const unsigned char *ival, int enc = 1)
  {
    num = 0;
    mode = m;
    encrypt = enc;
    ...
    if (mode == mode_cbc)
    {
      block_size = 16;
      iv_len = 16;
      memcpy(oiv, ival, iv_len);
      memcpy(iv, oiv, iv_len);
    }
  ...
  {
    if (mode == mode_cbc)
      return __cbc_cipher(out, in, inl);
    else if (mode == mode_ecb)
      return __ecb_cipher(out, in, inl);
    else if (mode == mode_ofb)
      return __ofb_cipher(out, in, inl);
    else if (mode == mode_cfb)
      return __cfb_cipher(out, in, inl);
    else if (mode == mode_ctr)
      return __ctr_cipher(out, in, inl);
    return -1;
  }
